                ...
                <form className="flex flex-col mt-10 gap-3 ">
                    <div className='flex flex-col'>
                        {cargando ? "" :
                            <select
                                name="categoria"
                                className={`bg-slate-300  placeholder-slate-500 rounded h-12 w-full pl-2 ${submitted && categoria == "" ? "border-red-600 border-l-2" : ""}`} value={categoria} onChange={handleCategoriaChange}
                            >
                                <option value="" className='p-2 rounded-full text-slate-500'>Categoría</option>
                                {categorias.map((categoria, i) => <option value={categoria} key={i} className='p-2 rounded-full'>{categoria}</option>)}
                            </select>
                        }
                    </div>
                    <div className='flex gap-3'>
                        <input
                            ...
                            className={`bg-slate-300  placeholder-slate-500 rounded h-12 w-full pl-2 ${submitted && values.duracion == "" ? "border-red-600 border-l-2" : ""}`} value={values.duracion}
                            onChange={handleInputChange}
                        />
                        <input
                            type="number" name="costo"
                            placeholder="Costo"
                            className={`bg-slate-300  placeholder-slate-500 rounded h-12 w-full pl-2 ${submitted && values.costo == "" ? "border-red-600 border-l-2" : ""}`} value={values.costo}
                            onChange={handleInputChange}
                        />
                    </div>
                    <div className='flex gap-3'>
                        ...
                        <select className={`bg-slate-300  placeholder-slate-500 rounded h-12 w-full pl-2 ${submitted && frecuencia == "" ? "border-red-600 border-l-2" : ""}`} onChange={handleFrecuenciaChange} defaultValue={frecuencia}>
                            <option value="" className='p-2 rounded-full text-slate-500'>Frecuencia</option>
                        ...
                        <select className={`bg-slate-300  placeholder-slate-500 rounded h-12 w-full pl-2 ${submitted && metodologia == "" ? "border-red-600 border-l-2" : ""}`} onChange={handleMetodologiaChange} defaultValue={metodologia}>
                            <option value="" className='p-2 rounded-full text-slate-500'>Metodologia</option>
                            <option value="Presencial" className='p-2 rounded-full '>Presencial</option>
                            <option value="Virtual" className='p-2 rounded-full '>Virtual</option>
                            <option value="Ambas" className='p-2 rounded-full '>Ambas</option>
                        </select>
                    </div>
                    <textarea
                        type="text" name='descripcion'
                        placeholder="Descripcion del servicio"
                        className={`bg-slate-300 placeholder-slate-500 rounded h-40 w-full pl-2 align-top ${submitted && values.descripcion == "" ? "border-red-600 border-l-2" : ""}`} value={values.descripcion}
                        onChange={handleInputChange}
                    />
