...
export default function Perfil() {
    ...
    useEffect(() => {
        if (!sesionIniciada()) {
            ...
            profesorPorId(localStorage.getItem("id")).then(data => setProfesor(data)).then(() => setCargando(false))
        }
    }, [])
    ...
    const modalHtml = <div className="fixed top-0 left-0 z-10 w-full h-full  bg-black bg-opacity-80 flex justify-center items-center  sm:py-28">
        <div className="flex flex-col bg-white rounded-3xl text-center items-center  p-3 sm:p-7 my-10 gap-3 relative h-full  max-sm:overflow-y-scroll">
            ...
            <div className='flex flex-col gap-3'>
                <div className='flex justify-between gap-3 max-sm:flex-col'>
                    ...
                    <input type="text" className='flex border rounded p-2' defaultValue={profesor.nombre} /></div>
                <div className='flex justify-between gap-3 max-sm:flex-col'><span className='font-bold my-auto'>Apellido: </span>
                    <input type="text" className='flex border rounded p-2' defaultValue={profesor.apellido} /></div>
                <div className='flex justify-between gap-3 max-sm:flex-col'><span className='font-bold my-auto'>Mail: </span>
                    <input type="text" className='flex border rounded p-2' defaultValue={profesor.mail} /></div>
                <div className='flex justify-between gap-3 max-sm:flex-col'><span className='font-bold my-auto'>Telefono: </span>
                    <input type="text" className='flex border rounded p-2' defaultValue={profesor.telefono} /></div>
                <div className='flex justify-between gap-3 max-sm:flex-col'><span className='font-bold my-auto'>Ubicación: </span>
                    <input type="text" className='flex border rounded p-2' defaultValue={profesor.ubicacion} /></div>
                <div className='flex justify-between gap-3 max-sm:flex-col'><span className='font-bold my-auto'>Imagen: </span>
                    <input type="text" className='flex border rounded p-2' defaultValue={profesor.img} />
                </div>
                <div className='flex justify-between gap-3 max-sm:flex-col'><span className='font-bold'>Experiencia: </span>
                    <textarea className='h-40 w-full border rounded p-2' type='text' defaultValue={profesor.experiencia} /></div>
            </div>
            <button className='flex bg-coral text-white border border-coral hover:bg-white hover:text-coral py-2 px-6 rounded-xl text-lg mt-auto' onClick={() => { handleGuardar() }}>Guardar</button>
        </div>
    </div>
            ...
            {cargando ? <Spinner /> :
                <>
                    ...
                    <div className='flex flex-col mx-3'>
                        <h2 className='text-4xl font-bold text-center mb-3 text-coral'>Contrataciones activas</h2>
