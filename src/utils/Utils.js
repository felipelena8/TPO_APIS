let estrellaLlena = (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 31 29" fill="none">
    <path
      d="M15.0591 0.164368L20.1579 8.7553L30.0591 10.8595L23.3091 18.2731L24.3296 28.1644L15.0591 24.1553L5.78857 28.1644L6.80908 18.2731L0.059082 10.8595L9.96031 8.7553L15.0591 0.164368Z"
      fill="#FFCB45"
    />
  </svg>
);
  ...
  for (let i in datos) {
    ...
    profesor.servicios = profesor.servicios.filter((elem) => {
      ...
      if (calificaciones.length != 0) {
        let redondeo = Math.round(calcularCalificacionServicio(elem));
        if (redondeo == 0) {
          redondeo = 1;
        }
  ...
  return (
    <div className="flex gap-2 h-4">
      {estrellas.map((estrella, i) => (
        <div
          className="flex h-7 mb-3 self-center center"
          key={i}
          value={i}
          onClick={(e) => {
            setCalificacion(i + 1);
...
function estrellasHtml(calificacion) {
  return calcularEstrellas(calificacion).map((elemento, i) => (
    <div className="flex h-full w-full" key={i}>
      {elemento == 1 ? estrellaLlena : elemento ? estrellaMitad : estrellaVacia}
    </div>
  ));
}
...
function formatearFecha(fechaString) {
  ...
  const [dia, mes, año] = fecha
    .toLocaleDateString(undefined, opcionesDeFormato)
    .split(" ");
  return `${dia} ${mes}. ${año}`;
}
...
function getContratacionesProfesor(profesor) {
  ...
  for (let i = 0; i < profesor.servicios.length; i++) {
    let contratacionesServicio = profesor.servicios[i].contrataciones;

    let contratacionesActivasServicio = contratacionesServicio.filter(
      (contratacion) => contratacion.estado === "Aceptado"
    );

    contrataciones = contrataciones.concat(contratacionesActivasServicio);
  }
